...
export function RecipeCard({id, image, name,score,ingredients, recipes}){
    ...
    async function bookMarkingToggle(e){
        ...
        const hasRecentIngredient = favoriteRecipes.some((fav) => {
            if(!fav.timestamp || !fav.ingredients){
                return false;
            }
            ...
            return(
                ...
                fav.ingredients.some(favIng => 
                    ingredientsArr && ingredientsArr.includes(favIng.toLowerCase().trim())
                )
            )
        })
    ...
    return(
        <div>
                                ...
                                {[...Array(5)].map((star, i) => {
                                    ...
                                    return  (
                                        <label key={ratingNumber}>
                                            ...
                                            <FaStar className = "star" color = {ratingNumber <= (hover ||ratingNumber) ? "var(--text-primary)" : "white"} size={30} onMouseEnter={() => setHover(ratingNumber)} onMouseLeave={() => setHover}/>
                                        </label>
                                    )
                                })}
                            </div>
            ...
            {openModal && <AddToMealPlan closeModal={setOpenModal} selectedRecipeId={id} selectedRecipeName={name} getMealPlans={getMealPlans} recipes={recipes}/>}  
